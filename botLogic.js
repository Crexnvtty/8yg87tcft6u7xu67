...
const T = {
  ...
  slotTaken: '⚠️ Ups! Ktoś właśnie zarezerwował ten termin. Wybierz inny:',
...
// Безопасное форматирование даты в YYYY-MM-DD БЕЗ конвертации в UTC
// (toISOString() может сдвинуть дату на день назад/вперёд около полуночи из-за часового пояса)
...
// Список месяцев от текущего до +N месяцев вперёд (N = config.maxMonthsAhead)
function getAvailableMonths() {
  const today = new Date();
  const months = [];
  const count = config.maxMonthsAhead || 6;
...
// Доступные даты внутри конкретного месяца — с учётом рабочих дней,
// уже прошедших дат, максимальной глубины записи и наличия хотя бы одного
// свободного слота (durationMin — длительность выбранной услуги)
function getAvailableDatesInMonth(year, month, durationMin) {
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  const absoluteMax = new Date(today);
  absoluteMax.setMonth(absoluteMax.getMonth() + (config.maxMonthsAhead || 6));

  const firstDay = new Date(year, month, 1);
  const lastDay = new Date(year, month + 1, 0);

  const dates = [];
  const cursor = new Date(Math.max(firstDay.getTime(), today.getTime()));
  while (cursor <= lastDay && cursor <= absoluteMax) {
    const weekday = cursor.getDay();
    if (config.workingHours[weekday]) {
      const iso = toISODateLocal(cursor);
      if (generateSlotsForDate(iso, durationMin).length > 0) {
        dates.push(iso);
      }
    }
...
function handleMessage(phone, rawText) {
  ...
  switch (step) {
    ...
    case 'choose_month':
      return handleChooseMonth(phone, text, data);
    case 'choose_date':
    ...
    case 'reschedule_choose_month':
      return handleRescheduleChooseMonth(phone, text, data);
...
function monthListText(months, headerText) {
  let msg = `${headerText}\n\n`;
  months.forEach((m, i) => {
    msg += `${i + 1}️⃣ ${MONTH_NAMES_PL[m.month]} ${m.year}\n`;
  });
  msg += `\n${T.sendNumber}`;
  return msg;
}
...
function handleChooseService(phone, text, data) {
  ...
  store.setState(phone, 'choose_month', { serviceId, months });
  return monthListText(months, '📅 *Wybierz miesiąc:*');
}

function handleChooseMonth(phone, text, data) {
  const idx = parseInt(text, 10) - 1;
  if (isNaN(idx) || idx < 0 || idx >= data.months.length) return T.invalidNumber;
  const { year, month } = data.months[idx];
  const service = store.getServiceById(data.serviceId);
  const duration = service ? service.duration_min : config.slotStepMinutes;
  const dates = getAvailableDatesInMonth(year, month, duration);

  if (dates.length === 0) {
    return (
      `Niestety, w ${MONTH_NAMES_PL[month]} nie ma wolnych terminów 😔\n\n` +
      monthListText(data.months, 'Wybierz inny miesiąc:')
    );
  }
  ...
  msg += `\n${T.sendNumber}`;
  store.setState(phone, 'choose_date', { serviceId: data.serviceId, dates });
  return msg;
}
...
function handleAskName(phone, text, data) {
  ...
  if (!result.success) {
    // Кто-то другой успел занять этот слот между показом списка и
    // подтверждением. Показываем клиенту актуальный список свободных
    // окон на ту же дату вместо простого отказа.
    const duration = service ? service.duration_min : config.slotStepMinutes;
    const slots = generateSlotsForDate(data.date, duration);
    if (slots.length === 0) {
      store.resetState(phone);
      return `${T.slotTaken}\n\n${T.noSlots(formatDateHuman(data.date))}\n\n` + T.welcome(config.businessName);
    }
    let msg = `${T.slotTaken}\n\n${T.chooseTime(formatDateHuman(data.date))}\n\n`;
    slots.forEach((s, i) => { msg += `${i + 1}️⃣ ${s}\n`; });
    msg += `\n${T.sendNumber}`;
    store.setState(phone, 'choose_time', { serviceId: data.serviceId, date: data.date, slots });
    return msg;
  }
...
function handleBookingAction(phone, text, data) {
  ...
  if (text === '1') {
    // Перенос — сначала выбираем месяц
    const months = getAvailableMonths();
    store.setState(phone, 'reschedule_choose_month', { bookingId: data.bookingId, months });
    return monthListText(months, '📅 *Wybierz miesiąc nowego terminu:*');
  }
...
function handleRescheduleChooseMonth(phone, text, data) {
  const idx = parseInt(text, 10) - 1;
  if (isNaN(idx) || idx < 0 || idx >= data.months.length) return T.invalidNumber;
  const { year, month } = data.months[idx];
  const booking = store.getBookingById(data.bookingId);
...
function handleRescheduleChooseTime(phone, text, data) {
  ...
  const result = store.updateBookingDateTime(data.bookingId, data.date, newTime);

  if (!result.success) {
    // Слот заняли между показом списка и подтверждением переноса.
    // Показываем актуальный список окон на ту же дату, старая запись
    // остаётся нетронутой (updateBookingDateTime не изменил её).
    const duration = service ? service.duration_min : config.slotStepMinutes;
    const slots = generateSlotsForDate(data.date, duration, data.bookingId);
    if (slots.length === 0) {
      store.resetState(phone);
      return `${T.slotTaken}\n\n${T.noSlots(formatDateHuman(data.date))}\n\n` + T.welcome(config.businessName);
    }
    let msg = `${T.slotTaken}\n\n${T.rescheduleChooseTime(formatDateHuman(data.date))}\n\n`;
    slot
function toHHMM(totalMin) {
  const h = Math.floor(totalMin / 60);
  const m = totalMin % 60;
